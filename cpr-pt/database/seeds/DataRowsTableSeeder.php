...
class DataRowsTableSeeder extends Seeder
{
     ...
     */
   private function createSessionsDataType($sessionsDataType){
      $dataRow = $this->dataRow($sessionsDataType, 'id');
     if (!$dataRow->exists) {
          $dataRow->fill([
             'type'         => 'number',
             'display_name' => 'ID',
     ...
     $dataRow = $this->dataRow($sessionsDataType, 'idUser');
      if (!$dataRow->exists) {
          $dataRow->fill([
      ...
      $dataRow = $this->dataRow($sessionsDataType, 'title');
     if (!$dataRow->exists) {
         $dataRow->fill([
             'type'         => 'text',
             'display_name' => 'Title',
     ...
     $dataRow = $this->dataRow($sessionsDataType, 'created_at');
     if (!$dataRow->exists) {
          $dataRow->fill([
             'type'         => 'timestamp',
     ...
     $dataRow = $this->dataRow($sessionsDataType, 'updated_at');
     if (!$dataRow->exists) {
          $dataRow->fill([
             'type'         => 'timestamp',
   ...
   private function createExercisesDataType($exercisesDataType){

      $dataRow = $this->dataRow($exercisesDataType, 'id');
     if (!$dataRow->exists) {
          $dataRow->fill([
             'type'         => 'number',
             'display_name' => 'ID',
     ...
     $dataRow = $this->dataRow($exercisesDataType, 'idSession');
      if (!$dataRow->exists) {
          $dataRow->fill([
                'type'         => 'select_dropdown',
                'display_name' => 'idSession',
                'required'     => 1,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
      ...
      $dataRow = $this->dataRow($exercisesDataType, 'time');
     if (!$dataRow->exists) {
         $dataRow->fill([
             'type'         => 'number',
             'display_name' => 'Time',
     ...
     $dataRow = $this->dataRow($exercisesDataType, 'recoil');
    if (!$dataRow->exists) {
        $dataRow->fill([
            'type'         => 'number',
            'display_name' => 'Recoil',
    ...
    $dataRow = $this->dataRow($exercisesDataType, 'compressions');
   if (!$dataRow->exists) {
      $dataRow->fill([
           'type'         => 'number',
           'display_name' => 'Compressions',
           'required'     => 1,
           'browse'       => 1,
           'read'         => 1,
           'edit'         => 1,
   ...
   $dataRow = $this->dataRow($exercisesDataType, 'hand_position');
 if (!$dataRow->exists) {
      $dataRow->fill([
          'type'         => 'number',
          'display_name' => 'Hand Position',
          'required'     => 1,
          'browse'       => 1,
          'read'         => 1,
          'edit'         => 1,
     ...
     $dataRow = $this->dataRow($exercisesDataType, 'created_at');
     if (!$dataRow->exists) {
          $dataRow->fill([
             'type'         => 'timestamp',
             'display_name' => 'created_at',
     ...
     $dataRow = $this->dataRow($exercisesDataType, 'updated_at');
     if (!$dataRow->exists) {
          $dataRow->fill([
             'type'         => 'timestamp',
             'display_name' => 'updated_at',
    ...
    public function run()
    {
        ...
        $sessionsDataType = DataType::where('slug', 'sessions')->firstOrFail();
        $exercisesDataType = DataType::where('slug', 'exercises')->firstOrFail();

        $this->createSessionsDataType($sessionsDataType);
        $this->createExercisesDataType($exercisesDataType);
